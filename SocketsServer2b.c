...
int main(int argc, char *argv[])
{
        ...
        while ( (n = read(newsockfd, buffer, 50, 0)) > 0)
        {
                if ((strncmp(&buffer, "@", 1) == 0)) break;
                strncat(fullname, &buffer, 1);
                printf("%s, %s\n", buffer, fullname);
        ...
        {
            nextletterstring[i] = fullname[i] + 1;
        }
        nextletterstring[strlen(nextletterstring)] = '@';

        for (int i = 0; i < strlen(nextletterstring); i++)
        {
            printf("Stlren = %i, nextletterstring[%i] = %c, nextletterstring = %s\n", strlen(nextletterstring), i, nextletterstring[i], nextletterstring);
            n = write(newsockfd, &nextletterstring[i], 1); // NB! Må bruke & for å hente ut et spesfikt element i buffer. 
